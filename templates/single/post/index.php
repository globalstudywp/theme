...
$sidebar           = globalstudy_post_sidebar_by_theme_options( $settings );

if ( is_active_sidebar( $sidebar ) && 'none' !== $sidebar || empty( $settings ) ) :
	?>
<div class="row <?php echo esc_attr( globalstudy_post_sidebar_position_theme_options( get_post_type( get_the_ID() ), $settings ) ); ?>">
...
if ( have_posts() ) {
	...
	while ( have_posts() ) :
		...
		globalstudy_post_author_by_theme_options( get_post_type( get_the_ID() ), $settings );
		globalstudy_post_comments_by_theme_options( get_post_type( get_the_ID() ), $settings );
	endwhile;
	echo '</main>';
} else {
