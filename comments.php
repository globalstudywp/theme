...
<div>
	...
	<?php if ( have_comments() ) : ?>
		...
		<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : ?>
			<nav class="navigation comment-navigation" role="navigation">
				<h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'globalstudy' ); ?></h2>
				<div class="nav-links">
					<?php
					$prev_link = get_previous_comments_link( esc_html__( 'Older Comments', 'globalstudy' ) );
					$next_link = get_next_comments_link( esc_html__( 'Newer Comments', 'globalstudy' ) );
					if ( $prev_link ) {
	...
	comment_form(
		array(
			...
			'label_submit'         => esc_html__( 'Submit', 'globalstudy' ),
		)
	);
	?>
	...
	if (
		...
		?>
		<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'globalstudy' ); ?></p>
		<?php
	endif;
	?>
